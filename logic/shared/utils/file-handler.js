...
function generateFilename(user, serviceType = "MIGRNW") {
  const now = new Date();
  ...
  return `${timestamp}_${serviceType}_${username}.xlsx`;
}
...
async function uploadFile(bot, roomId, processedResult, user, filename = null, serviceType = "MIGRNW") {
  try {
    // Generate filename if not provided
    const outputFilename = filename || generateFilename(user, serviceType);
    
    logR2CCW(user, outputFilename, 'Starting file upload');
       
    // Create temporary file with appropriate extension
    const tempFile = path.join(__dirname, outputFilename);
